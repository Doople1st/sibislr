...
def generator(type):
  ...
  if type=="train":
    ...
    while True:
      batch_features = np.zeros((1,NUM_FRAMES, FRAME_WIDTH, FRAME_HEIGHT,1))
      temp = np.zeros((BATCH,NUM_FRAMES, FRAME_WIDTH, FRAME_HEIGHT))
...
#len(hf["train"])
#len(hf["validation"])
rgb_model.fit_generator(generator("train"), steps_per_epoch=60//BATCH, epochs=40, callbacks=callbacks_list,shuffle=True,validation_data = generator("validation"),validation_steps=40//BATCH)
